...
| `teleport_audit_athena_batch_processing_seconds`     | histogram   | Teleport Audit Log  | Duration of processing single batch of events in athena audit log.                                 |
| `teleport_audit_athena_s3_flush_seconds`             | histogram   | Teleport Audit Log  | Duration of flushing parquet files to s3 in athena audit log.                                      |
| `teleport_audit_athena_delete_events_seconds`        | histogram   | Teleport Audit Log  | Duration of deletion events from SQS in athena audit log.                                          |
| `teleport_audit_athena_batch_size`                   | histogram   | Teleport Audit Log  | Overall size of events in single batch in athena audit log.                                        |
| `teleport_audit_athena_batch_count`                  | counter     | Teleport Audit Log  | Total number of events in single batch in athena audit log.                                        |
| `teleport_audit_athena_last_processed_timestamp`     | gauge       | Teleport Audit Log  | Number of last processing time in athena audit log.                                                |
| `teleport_audit_athena_age_oldest_processed_message` | gauge       | Teleport Audit Log  | Number of age of oldest event in athena audit log.                                                 |
| `teleport_audit_athena_errors_from_collect_count`    | counter     | Teleport Audit Log  | Number of collect failures in athena audit log.                                                    |
