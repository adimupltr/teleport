...
func metricConsumerBatchProcessingDuration(batchInterval time.Duration) prometheus.Histogram {
	...
	return prometheus.NewHistogram(
		prometheus.HistogramOpts{
			...
			Name:      teleport.MetricAthenaConsumerBatchPorcessingDuration,
			Help:      "Duration of processing single batch of events in athena events",
...
var (
	consumerS3parquetFlushDuration = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerS3FlushDuration,
			Help:      "Duration of flush and close of s3 parquet files in athena events",
	...
	consumerDeleteMessageDuration = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerDeleteEventsDuration,
			Help:      "Duration of delation of events on SQS in athena events",
			// lowest bucket start of upper bound 0.001 sec (1 ms) with factor 2
			// highest bucket start of 0.001 sec * 2^15 == 32.768 sec
			Buckets: prometheus.ExponentialBuckets(0.001, 2, 16),
		},
	...
	consumerBatchSize = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerBatchSize,
			Help:      "Size of single batch of events in athena events",
	...
	consumerBatchCount = prometheus.NewCounter(
		prometheus.CounterOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerBatchCount,
			Help:      "Number of events in single batch in athena events",
		},
	)
	...
	consumerLastProcessedTimestamp = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerLastProcessedTimestamp,
	...
	consumerAgeOfOldestProcessedMessage = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerOldestProcessedMessage,
	...
	consumerNumberOfErrorsFromSQSCollect = prometheus.NewCounter(
		prometheus.CounterOpts{
			Namespace: teleport.MetricNamespace,
			Name:      teleport.MetricAthenaConsumerCollectFailed,
