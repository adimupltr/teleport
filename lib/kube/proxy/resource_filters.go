...
func newResourceFilterer(allowedResources, deniedResources []types.KubernetesResource, log logrus.FieldLogger) responsewriters.FilterWrapper {
...
func (d *resourceFilterer) FilterObj(obj runtime.Object) (bool, bool, error) {
	...
	case *corev1.Pod:
		err := filterResource(o, d.allowedResources, d.deniedResources)
...
func filterResourceList[T kubeObjectInterface](originalList []T, allowed, denied []types.KubernetesResource, log logrus.FieldLogger) []T {
	filteredList := make([]T, 0, len(originalList))
	for _, resource := range originalList {
		if err := filterResource(resource, allowed, denied); err == nil {
...
type kubeObjectInterface interface {
	kubeObjectPartialMetadataInterface
	GetNamespace() string
	GetName() string
}

// kubeObjectPartialMetadataInterface is an interface that all Kubernetes
// objects must implement to be able to filter them. It is used to extract
// the kind of the object from the GroupVersionKind object.
type kubeObjectPartialMetadataInterface interface {
	GroupVersionKind() schema.GroupVersionKind
}

// filterResource validates if the user should access the current resource.
func filterResource[T kubeObjectInterface](resource T, allowed, denied []types.KubernetesResource) error {
	err := matchKubernetesResource(
		types.KubernetesResource{
			Kind:      getKindFromGVK(resource),
...
// getKindFromGVK returns the kind of the object in lowercase.
func getKindFromGVK[T kubeObjectPartialMetadataInterface](obj T) string {
	return strings.ToLower(obj.GroupVersionKind().Kind)
}
