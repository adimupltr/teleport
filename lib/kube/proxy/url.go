...
func getResourceFromRequest(requestURI string) *types.KubernetesResource {
	apiResource := parseResourcePath(requestURI)
	resource := getResourceFromAPIResource(apiResource.resourceKind)
	resourceType, ok := allowedResources[allowedResourcesKey{apiGroup: apiResource.apiGroup, resourceKind: resource}]
